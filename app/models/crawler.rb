...
class Crawler < ActiveRecord::Base
  ...
  def self.add_page_to_docs(page, docs, page_name)
    ...
    page_for_doc = ActionController::Base.helpers.strip_tags(page_for_doc)
    page_for_doc.squish!
    page_for_doc.gsub!(/\n\n+/, "\n")

    tokens = %w(pt cea ha fa ne ului 06a106 post_img lor header_wrapper content_wrapper widgettitle 1st 2nd 3rd 4th poate nici caption_cat  pagination il am oh fi ok tot ts fie span 1px se mi th au li le iti isi sc dar ar nr h2 h1 h3 h4 h5 h6 h7 ap sidebar_widget nivo_caption_wrapper srl ti lu tab ia tr cg al la cei cl ta quot javascript ma dupa ci browser ca ai la te ron asa da cu nu te si va ce cum unde o un ala ul urile a de la cu peste langa sub catre prin contra in pe fara pentru din asemenea asa noastra toate tau eu tu el ea noi voi ei ele mei sau sa acesta asta aceea același cine ce care cât care ceea cine unul unii cineva altul oricare vreunul)
